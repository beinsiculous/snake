use insiculous_snake::{game_config, SnakeGame};
...
fn main() {
    ...
    let config = game_config(&root.join("assets").to_string_lossy())
        .with_achievement_save_path(root.join("saves/snake_achievements.json").to_string_lossy())
        .with_input_settings_path(root.join("saves/input_settings.json").to_string_lossy())
        .with_score_save_path(root.join("saves/snake_scores.json").to_string_lossy());
    ...
    #[cfg(feature = "editor")]
    editor_integration::run_game_with_editor(SnakeGame::default(), config).unwrap();
    #[cfg(not(feature = "editor"))]
    engine_core::prelude::run_game(SnakeGame::default(), config).unwrap();
}
